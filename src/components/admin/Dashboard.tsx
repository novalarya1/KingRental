...
export interface BookingRecord {
  ...
  user: string;
  unit: string;
  // Status sebaiknya memiliki tipe spesifik agar autocomplete jalan
  status: 'pending' | 'paid' | 'active' | 'completed' | 'cancelled' | string;
  price: string | number;
  date: string;
  branch: string;
}
...
const getStatusStyles = (status: string) => {
  ...
  if (s.includes('active') || s.includes('paid')) return 'bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20';
  if (s.includes('completed') || s.includes('finished')) return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20';
  if (s.includes('cancelled') || s.includes('failed')) return 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-500/20';
  if (s.includes('pending')) return 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20 hover:bg-yellow-500/20';
  return 'bg-zinc-800 text-zinc-400 border-white/5 hover:border-white/20';
};
...
export default function Dashboard({ 
  ...
  // Opsi status disesuaikan dengan logic backend (Laravel biasanya lowercase/snake_case di DB, tapi uppercase di UI)
  const statusOptions = ['Pending', 'Paid', 'Active', 'Completed', 'Cancelled'];

  // Kalkulasi Statistik
  const stats = useMemo(() => {
    // Helper parse local
    const parseVal = (price: string | number) => typeof price === 'string' ? parseFloat(price.replace(/[^0-9.-]+/g, "")) : price;
    
    const totalRevenue = bookings.reduce((acc, curr) => acc + (parseVal(curr.price) || 0), 0);
    
    const activeRentals = bookings.filter(b => {
      const s = b.status?.toLowerCase() || '';
      return s === 'active' || s === 'on_rent' || s === 'paid'; // Sesuaikan logic bisnis
    }).length;
    ...
    return {
      revenue: totalRevenue >= 1000000000 
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={5} className="px-10 py-32 text-center">
                      ...
                      <p className="text-zinc-600 text-[10px] font-bold uppercase mt-4 tracking-widest">Decrypting Data...</p>
                    </td>
                  </tr>
                      ...
                      <td className="px-10 py-6">
                        <div className="flex flex-col gap-3">
                           <div className="flex items-center gap-1.5 text-zinc-500">
                             ...
                             <span className="text-[10px] font-bold uppercase tracking-tighter">{booking.branch || 'Main Branch'}</span>
                           </div>
                           ...
                           <div className="relative w-fit group/select">
                             <select 
                               ...
                               // Menggunakan fungsi wrapper agar onUpdateStatus aman
                               onChange={(e) => {
                                 if (onUpdateStatus) {
                               ...
                               className={`appearance-none cursor-pointer pl-3 pr-8 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest border transition-all outline-none disabled:opacity-50 disabled:cursor-not-allowed
                                 ${getStatusStyles(booking.status || 'pending')}`}
                             >
