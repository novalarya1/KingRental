    ...
    <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-8 animate-bounce">
          ...
          <span className="text-[10px] font-black tracking-[0.2em] text-zinc-400 uppercase">Latest Units Available</span>
        </div>
        ...
        <p className="text-zinc-500 max-w-2xl mx-auto text-lg md:text-xl font-medium mb-12">
          Premium vehicle rental service with instant booking and elite fleet selection.
        </p>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 bg-zinc-900/50 p-4 rounded-[2.5rem] border border-white/10 backdrop-blur-md max-w-3xl mx-auto">
          <div className="flex gap-2 w-full md:w-auto">
            <button 
              ...
              className={`flex-1 md:flex-none px-8 py-4 rounded-2xl font-bold text-sm transition-all ${
                currentFilter === 'All' ? 'bg-white text-black' : 'text-zinc-400 hover:bg-white/5'
              }`}
            >
              All
            </button>
            <button 
              ...
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-sm transition-all ${
                currentFilter === 'Car' ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:bg-white/5'
              }`}
            >
              <Car size={18} /> Car
            </button>
            <button 
              onClick={() => onFilterChange('Motorcycle')}
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-bold text-sm transition-all ${
                currentFilter === 'Motorcycle' ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:bg-white/5'
              }`}
            >
          ...
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/5 py-4 pl-12 pr-4 rounded-2xl text-sm focus:outline-none focus:border-blue-500 transition-all text-white placeholder:text-zinc-600"
            />
          </div>
        </div>

        <div className="mt-20 animate-bounce opacity-20">
          <ArrowDown className="mx-auto text-white" />
        </div>
      </div>
    </section>
  );
